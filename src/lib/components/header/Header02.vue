    ...
    methods: {
      async fetchUnreadNewsItems() {
        try {
          ...
          // Get the IDs of read articles from local storage
          const readArticles = JSON.parse(localStorage.getItem('readArticles')) || [];

          this.newsItemsThisMonth = this.newNewsItems.filter((doc) => {
            const messageDate = new Date(doc.data.datum);

            // Check if article is within the last month and not read
            const isWithinOneMonth = messageDate >= oneMonthAgo;
            const isUnread = !readArticles.includes(doc.id);
